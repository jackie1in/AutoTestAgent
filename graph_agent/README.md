...
- **测绘**：用 browser-use Agent 探索登录/业务流程，记录每一步的 selector、动作与语义意图，生成 DiGraph。
- **图谱**：图持久化为 JSON（节点、边含 selector / action / semantic_label / data_key），供前端展示与寻径使用。
- **寻径**：根据用户选择的意图（semantic_label）在图中找到从起点到目标状态的路径。
...
可选参数：`--url`（起始 URL）、`--output`（图谱 JSON 路径）、`--inventory`（清单 JSON 路径，默认 `graph_agent/data/element_inventory.json`）。
...
1. **图谱**：页面顶部展示当前加载的图（`graph_agent/data/graph.json`），节点为状态/URL，边为步骤（selector、动作、语义标签）。
2. **意图**：从下拉框选择一条语义意图（对应边的 `semantic_label`），用于寻径。
3. **测试数据**：在表单中填写回放时需要的字段（如 username、password），与图中的 `data_key` 对应。
