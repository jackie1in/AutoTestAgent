...
class ScoutElement(BaseModel):
    """Interactive element extracted from scout report."""
    selector: str = Field(description="Playwright selector, e.g. #id or [name='x'] or xpath=...")
    type: str = Field(description="Element type: button, input, link, richtext, or other")
    label: str | None = Field(default=None, description="Optional short description")


class ScoutElementList(BaseModel):
    """List of interactive elements from a web page."""
    elements: list[ScoutElement] = Field(default_factory=list, description="Array of interactive elements")
...
def _setup_browser_use_timeouts():
    ...
    if mapping_timeout:
        try:
            ...
            LOG.debug(f"Set browser-use timeouts to {timeout_val}s")
        except ValueError:
            pass

SCOUT_TASK_TEMPLATE = (
    "TARGET URL: {url} - YOU MUST NAVIGATE TO THIS URL FIRST! "
    "ACTION 1: Use the navigate action to go to {url} "
    "ACTION 2: After navigation completes, identify ALL interactive elements on the page. "
    "Include rich text editors (contenteditable divs, TinyMCE, CKEditor, Quill, ProseMirror) as type 'richtext'. "
    "Do NOT click anything except the navigate action. "
...
async def run_scout(
    ...
    LOG.info("[DEBUG] Scout task: %s", task)
    ...
    # Debug: Verify task is accessible
    LOG.info("[DEBUG] Agent task set: %s", getattr(agent, 'task', 'NOT FOUND'))

    # Register browser for global cleanup (Ctrl+C handling)
    try:
        from graph_agent.cartography.runner import _register_browser, _unregister_browser
        _register_browser(browser)
    except ImportError:
        pass  # runner not available, local cleanup only
    ...
    try:
        ...
        try:
            ...
            # Unregister browser from global cleanup
            try:
                from graph_agent.cartography.runner import _unregister_browser
    ...
    if not raw_report:
        ...
        if elements:
            ...
            try:
                extraction_result = await ainvoke_structured(
                    llm,
                    system_prompt="You are a web page element extractor. Extract interactive elements from the scout report and return valid JSON.",
                    user_prompt=EXTRACT_PROMPT.format(report=raw_report),
                    output_format=ScoutElementList,
                )
                elements = [
                    {"selector": e.selector, "type": e.type, "label": e.label}
                    for e in extraction_result.elements
                ]
                extraction_path = "llm_structured"
            except Exception as e:
