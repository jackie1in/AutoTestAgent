...
class ReActExplorer(BaseAgent):
    ...
    def __init__(
        ...
        # Extra actions supported beyond BaseAgent defaults
    ...
    def _build_user_prompt(
        ...
        if effective_zone_selectors and self._explore_start_url:
            _current_clean = (
                _extract_spa_route(current_url)
                or current_url.split("?")[0].split("#")[0]
            )
            _start_clean = (
                _extract_spa_route(self._explore_start_url)
                or self._explore_start_url.split("?")[0].split("#")[0]
            )
    ...
    # Actions dispatched via browser-use Tools.act()
    _BROWSER_USE_ACTIONS = frozenset({
        "click", "input", "select_dropdown", "scroll",
        "wait", "go_back", "done",
        "send_keys", "evaluate",
        "dropdown_options", "find_elements", "search_page",
    })
    ...
    async def _execute_action(self, action_type: str, params: dict[str, object]) -> str:
        ...
        # Track explored indices for click/input/select_dropdown
        ...
        try:
            # Track accumulative wait time for observation feedback
            if action_type == "wait":
                seconds = min(cast(int, params.get("seconds", 1)), 10)
                self._total_wait_time += seconds

            # ── Browser-use native actions → delegate to Tools.act() ──
            if action_type in self._BROWSER_USE_ACTIONS:
                return await self._execute_browser_use_action(action_type, params)

            # ── Project-specific actions ───────────────────────────────
            match action_type:
                case "scroll_horizontally":
                    direction = cast(str, params.get("direction", "right"))
                    amount = cast(int, params.get("amount", 300))
                ...
                case "solve_captcha":
                    return await self._handle_captcha_action(params, controller, browser)
                # Backward-compat: execute_javascript → evaluate
    ...
    async def _execute_browser_use_action(
        ...
        """Delegate action execution to browser-use Tools.act()."""
        if action_type not in self.tools.registry.registry.actions:
            return f"Action '{action_type}' not found in browser-use registry"

        action_info = self.tools.registry.registry.actions[action_type]
        try:
            validated_params = action_info.param_model(**params)
        except Exception as e:
            return f"Invalid params for {action_type}: {e}"

        # Build a single-action model via Tools.act()
        action_model_cls = self.tools.registry.create_action_model(
            include_actions=[action_type]
        )
        action_instance = action_model_cls(**{action_type: validated_params})
        result = await self.tools.act(action_instance, browser_session=self.browser)
        return result.extracted_content or str(result)

    async def _handle_captcha_action(
        self,
        params: dict[str, object],
        controller: PageController,
        browser: Browser,
    ) -> str:
        """Handle solve_captcha action (complex manual/auto logic kept separate)."""
        page = await browser.get_current_page()
        if page is None:
            return "CAPTCHA_FAILED no_active_page"
        _input_index_raw = params.get("input_index")
        input_index: int | None = (
            cast(int, _input_index_raw) if _input_index_raw is not None else None
        )
            ...
            for _attempt in range(1, _MAX_MANUAL_RETRIES + 1):
                ...
                if not captcha_code:
                    if _attempt < _MAX_MANUAL_RETRIES:
                        logger.info("[CAPTCHA_FLOW] manual_empty, asking again")
                        continue
                    result_text = (
                ...
                if _ok or _attempt >= _MAX_MANUAL_RETRIES:
                    ...
                    return result_text
                logger.info(
                    "[CAPTCHA_FLOW] manual_fill_failed attempt=%d, pausing for re-entry",
                    _attempt,
                )
