...
Uses OpenAI SDK with KIMI's OpenAI-compatible API.
"""
...
def _strip_image_detail(messages: list[Any]) -> list[Any]:
    """Remove the unsupported 'detail' field from image_url content parts.

    Kimi's API follows the OpenAI multimodal spec but does NOT support the
    optional ``detail`` field inside ``image_url`` objects.  Sending it causes
    the request to be rejected.  This helper strips it from the already-
    serialised message list (list[dict]) in-place and returns the same list.
    """
    for msg in messages:
...
class ChatKimi(BaseChatModel):
    ...
    Supports KIMI-specific features:
    ...
    def _build_extra_body(self) -> dict[str, Any] | None:
        """Build extra_body with KIMI-specific parameters."""
        extra: dict[str, Any] = {}
    ...
    def _extract_thinking(self, response: ChatCompletion) -> str | None:
        ...
        KIMI-5.1 uses message.model_extra['reasoning'] or message.model_extra['reasoning_content']
        """
        ...
        if hasattr(message, "reasoning_content") and getattr(
            ...
            logger.debug(f"[KIMI Thinking] {str(thinking)[:200]}...")
            return thinking

        # Method 2: model_extra['reasoning'] (KIMI-5.1 vLLM)
        if hasattr(message, "model_extra") and message.model_extra:
            if "reasoning" in message.model_extra:
                thinking = message.model_extra["reasoning"]
                logger.debug(f"[KIMI Thinking] {thinking[:200]}...")
                return thinking
    ...
    async def _parse_stream_response(
        ...
        """Parse streaming response from KIMI.
        ...
        async for chunk in stream:
            ...
            # Collect thinking/reasoning (KIMI-specific)
            # KIMI-5.1 vLLM uses delta.reasoning
    ...
    def _parse_non_stream_response(
        ...
        """Parse non-streaming response from KIMI."""
        ...
        if output_format is not None:
            ...
            if choice.message.content is None:
                raise ModelProviderError(
                    message="KIMI response missing content",
                    status_code=500,
                    model=self.name,
                )
            ...
            try:
                ...
                logger.warning(
                    "[KIMI Structured Debug] parse failed | model=%s schema=%s required=%s raw_output=%r err=%s",
                    self.model,
    ...
    async def ainvoke(
        ...
        **kwargs: Any,
        ...
        """Invoke KIMI API with stream or non-stream mode.
        ...
        try:
            ...
            if output_format is None:
                KIMI_messages = OpenAIMessageSerializer.serialize_messages(messages)
                response_format = None
            else:
                modified_messages = self._inject_format_instructions(
                    messages, output_format
                )
                KIMI_messages = OpenAIMessageSerializer.serialize_messages(
                    modified_messages
                )
                # Kimi json_schema response_format requires the full schema object.
                response_format = {
                    "type": "json_schema",
                    "json_schema": {
            ...
            # Kimi does not support the 'detail' field inside image_url.
            # Strip it from every content part to avoid API errors.
            KIMI_messages = _strip_image_detail(KIMI_messages)

            # response_format is either None or a dict; cast to Any to satisfy
            # the OpenAI SDK's ResponseFormat union type annotation.
