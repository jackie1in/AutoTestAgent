...
PRD Task 1: 本轮验收默认使用 uv run python -m graph_agent.mapping.run 输出的数据文件。

用法:
  uv run python graph_agent/run_e2e_acceptance.py [--graph PATH] [--re-infer] [--url URL]

  1. 若提供 --graph，从该文件加载图；否则使用 graph_agent/data/graph.json（默认来自 mapping.run 输出）
...
def _find_path_length_ge_2(
    graph, intent_queries: list[str] | None = None
) -> tuple[list, str | None]:
...
async def run_acceptance(
    ...
    if graph.number_of_edges() == 0:
        print(
            "图为空，无法验收。请先运行 mapping: uv run python -m graph_agent.mapping.run --url <url>"
        )
        return False
    ...
    print(
        f"统计: edge_count={stats_before['edge_count']}, "
    ...
    if re_infer and stats_before["intent_missing_count"] > 0:
        ...
        print(
            f"Re-infer: total={re_stats['total']}, succeeded={re_stats['succeeded']}, failed={re_stats['failed']}"
        )
        graph = load_graph(path_obj)
        stats_after = _compute_stats(graph)
        print(
            f"改进后: intent_missing={stats_after['intent_missing_count']}, "
    ...
    if not path:
        ...
        return False
    print(
        f"找到路径 (intent={intent_used}, len={len(path)}): {path[0].source} -> ... -> {path[-1].target}"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--re-infer", action="store_true", help="对 intent=null 边执行重推"
    )
    parser.add_argument("--url", default=None, help="回放起始 URL（默认从图推断）")
    args = parser.parse_args()

    ok = asyncio.run(
        run_acceptance(args.graph, re_infer=args.re_infer, start_url=args.url)
    )
